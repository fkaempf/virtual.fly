...
# Main stimulus: fullscreen on rightmost monitor (pygame)
...
mapx = np.load('D:/screen.calibration/configs/camera.projector.mapping/mapx.experimental.npy').astype(np.float32)
mapy = np.load('D:/screen.calibration/configs/camera.projector.mapping/mapy.experimental.npy').astype(np.float32)

# main stimulus resolution
...
# pygame window on rightmost monitor, fullscreen scaled
right_mon = pick_rightmost_monitor()
os.environ.setdefault("SDL_AUDIODRIVER", "dummy")
os.environ.setdefault("SDL_VIDEODRIVER", "windows")
os.environ.setdefault("SDL_HINT_VIDEO_HIGHDPI_DISABLED", "1")
os.environ["SDL_VIDEO_WINDOW_POS"] = f"{right_mon.x},{right_mon.y}"

pygame.init()
screen = pygame.display.set_mode((STIM_W, STIM_H), pygame.FULLSCREEN | pygame.SCALED)
pygame.display.set_caption("fly stimulus")
clock = pygame.time.Clock()
font = pygame.font.SysFont("Arial", 16)

# OpenCV minimap window on primary (main) screen at (50,50)
...
while running:
    ...
    screen.blit(font.render(info, True, (220, 220, 220)), (10, STIM_H - 24))
    pygame.display.flip()
